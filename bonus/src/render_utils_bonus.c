...
/*   Updated: 2025/12/12 12:59:17 by mamarti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	lite_render(t_game *game, t_pos next_pos, char t_tile, int kc)
{
	...
	player = p_render(kc);
	next_img = game->p_cur++ % 2;
	put_sprite(game, &game->fl_img, (t_pos){2, 1});
	if (player_tile == EXIT)
	...
	if (t_tile == EXIT)
		...
		put_sprite(game, &game->pl_img[player][next_img], next_pos);
}
		...
		while (pos.x < game->map_data->width)
		{
			...
			else
			{
				...
				if (game->map_data->map[pos.y][pos.x] == PLAYER)
					put_sprite(game, &game->pl_img[0][game->p_cur++ % 2], pos);
					...
					put_sprite(game, &game->co_img[game->c_cur++ % 3], pos);
			}
			pos.x++;
		}
		pos.y++;
