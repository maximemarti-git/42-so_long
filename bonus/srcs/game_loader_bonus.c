...
/*   Updated: 2025/12/12 15:02:40 by mamarti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_data(t_map *map, t_game *game)
{
	...
	game->mobs_nb = map->mobs;
	game->p_pos = map->s_pos;
	game->moves = 0;
	game->mobs_data = (t_mobs *)malloc(sizeof(t_mobs) * game->mobs_nb);
	if (!game->mobs_data)
		p_error_map(MOBS_MAL_ERR, game->map_data);
	game->mobs_data->texture = 0;
	game->last_update_ms = get_ctms();
}
